 ...
 *
 * This template is used for displaying the content of an audio post-format.
 *
 * @package Alku
 * @subpackage Template
 * @since 0.1.0
        ...
        <article id="post-<?php the_ID(); ?>" class="<?php hybrid_entry_class(); ?>">
          ...
          <header class="entry-header">
            <?php if ( is_singular( 'post' ) ) : ?>
              <?php echo apply_atomic_shortcode( 'entry_title', the_title( '<i class="icon-headphones"></i><h1 class="entry-title">', '</h1><!-- .entry-title -->', false ) . '[entry-edit-link]' ); ?>
            <?php else : ?>
              <?php echo apply_atomic_shortcode( 'entry_title', '<i class="icon-headphones"></i>[entry-title tag="h1"] [entry-edit-link]' ); ?>
